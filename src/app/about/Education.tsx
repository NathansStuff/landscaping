...
import { EducationDetails } from '@/components/EducationPoints';
...
function Education({ educations }: EducationProps) {
  ...
  return (
    <div className='my-32 md:my-16 w-[75%]'>
      <h2 className='font-bold text-8xl mb-32 w-full text-center md:text-6xl xs:text-4xl md:mb-8'>Education</h2>
      <div ref={ref} className='w-full mx-auto relative lg:w-[90%] sm:w-full'>
        ...
        <ul className='w-full flex flex-col items-start justify-between ml-4 xs:ml-2'>
          {educations.map((education) => {
            return <EducationDetails education={education} key={education.title} />;
          })}
        </ul>
      </div>
    </div>
